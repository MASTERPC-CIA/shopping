...
//        $this->load->library("common/product");//load library//Cargamos las librerías
    ...
    public function index() {
        ...
        $productos_remate = $this->generic_model->get_in(
                ...
                $where_in_prod, 
                $where_not_in = null,
                'codigo,nombreUnico,stockactual,costopromediokardex,productogrupo_codigo',
                $group_by = null,
                array('stockactual' => 'desc')
        );
        if(!empty($this->user->id)){
            ...
            $datac['clientetipo_precios'] = $this->generic_model->get_data(
                'billing_clientetipotiposprecio',
                array('clientetipo_idclientetipo ' => $datac['tipocliente']->idclientetipo)
            );
        ...
        $all_product = $this->generic_model->get_all(
                'billing_producto',
                'codigo,nombreUnico,stockactual',
                array('stockactual' => 'desc')
        );
        ...
        $rest_product = $this->generic_model->count_all_results(
                'billing_producto',
                '',
                array('stockactual' => 0, 'costopromediokardex' => 0.0, 'esServicio' => 1)
        );
    ...
    private function products_in_bod($productos) {
        ...
        foreach ($productos as $key => $value) {
            $bodegas_stock = $this->generic_model->get_data(
                    'billing_stockbodega',
                    array('producto_codigo' => $value->codigo),
...
//        $datah['nombreproducto'] = $name_prod;
        if (!empty($idgrupo)) {
            ...
            $productoxgrupo = $this->generic_model->get_data(
                    'billing_producto',
                    array('productogrupo_codigo' => $idgrupo, 'stockactual !=' => 0, 'costopromediokardex !=' => 0.0, 'esServicio' => 0),
                    'codigo,nombreUnico,stockactual,costopromediokardex'
            );
            $this->productos_model->paginacion(
            ...
            foreach ($productos as $key => $value) {
                ...
                if(!empty($this->user->id)){
                    ...
                    $precios = $this->product->get_precio_prod($value->codigo,$datah['data_client']->default_price);
                    $desc = $datah['data_client']->descuento;
                }else{
                    $desc = 20.00;
                    $precios = $this->product->get_precio_prod($value->codigo,'pB');
                }
        ...
        if ($productos) {
            foreach ($productos as $key => $value) {
                ...
                if(!empty($this->user->id)){
                    ...
                    $desc = $data['data_client']->descuento;
                }else{
                    $desc = 20.00;
                    $precios = $this->product->get_precio_prod($value->codigo,'pB');
                }
        ...
        if ($productos) {
            foreach ($productos as $key => $value) {
                ...
                if(!empty($this->user->id)){
                    ...
                    $precios = $this->product->get_precio_prod($value->codigo,$data['data_client']->default_price);
                    $desc = $data['data_client']->descuento;
                }else{
                    $desc = 20.00;
                    $precios = $this->product->get_precio_prod($value->codigo,'pB');
...
//        $this->load->library("Email");
        ...
        if ($cart = $this->cart->contents()) {
            ...
            $total_sin_desc = $this->cart->format_number($this->cart->total());
            ...
            $this->table->add_row('', '', 'IVA 12%: ', number_decimal($iva));
...
//                $correos_to = array($empleado->email, $email_usuario, 'nvallejo@masterpc.com.ec');
            } else {
               $correos_to = array('dannyjimenez110@gmail.com');
//                $correos_to = array('masterpc@masterpc.com.ec', $email_usuario, 'nvallejo@masterpc.com.ec');
            }
