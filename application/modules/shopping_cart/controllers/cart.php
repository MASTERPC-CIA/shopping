...
//            $pago = $this->input->get('pago');
        ...
        $datac['total_sin_desc'] = $this->cart->total();
        $datac['total_con_desc'] = $datac['total_sin_desc'] - $datac['diferencia'];
        $datac['iva'] = $datac['total_con_desc'] * get_settings('IVA') / 100;
...
//        $stock = $this->product->get_stock_disponible($id);
        $bodegas_stock = $this->generic_model->get_data(
                'billing_stockbodega', array('producto_codigo' => $id)
        );
        $bodegas_id = '';
...
/// fin de la método  add
    ...
    function update_cart() {
        ...
        $producto = $this->generic_model->get_by_id(
                'billing_producto', $fila[2], array('codigo', 'nombreUnico', 'stockactual', 'productogrupo_codigo'), 'codigo'
        );
