...
//echo lineBreak(1, array('class'=>'clr'));
    ...
    if($idgrupo && $name_prod){
        echo Open('div');
            echo tagcontent('a', '<span class="glyphicon glyphicon-arrow-down"></span>Descargar PDF', array('href'=>base_url('shopping_cart/productos/create_pdf/'.$idgrupo.'/'.$name_prod), 'target'=>'_blank','class'=>'btn btn-danger btn-xs'));
        echo Close('div');
    }
        ...
        echo Open('div',array('class'=>'tab-pane active','id'=>'blockView'));
            ...
            if(!empty($productos || $producto_nombre)){
                ...
                foreach ($arreglo as $prod){
					...
					echo Open('div',array('class'=>'col-md-4 pull-left', 'style'=>'min-height:300px;overflow:hidden'));
					
				
                        $imagenbloc = tagcontent('img', '', array('src' => $imagencargar, 'alt' => $prod->codigo, 'style' => 'width: 200px; height: 230px;'));
                            echo Open('div',array('class'=>'owl-item cloned','style'=>'width: 236.667px; margin-right: 20px;'));
                                ...
                                echo Open('div',array('class'=>'product-f-image'));
                                if($prod->stock_bodega == 0){
                                        $danger= tagcontent('span','AGOTADO',array('class'=>'badge'));
                                    }else{
                                        $danger = '';
                                    }
                                //echo $imagenbloc.$danger;
                                echo tagcontent('a',$imagenbloc . $danger ,array('href'=>  base_url('shopping_cart/productos/product_detail?codigo='.$prod->codigo)));
                                echo Open('div',array('class'=>'product-hover'));
                                     //   echo Open('form',array('method'=>'post','action'=>  base_url('shopping_cart/cart/add_product')));
                                        echo tagcontent('a', 'Ver detalles'. tagcontent('i','',array('class'=>'fa fa-link')),array('href'=> base_url('shopping_cart/productos/product_detail?codigo='.$prod->codigo),'class'=>'view-details-link'));
                                       
                                        echo input(array('type'=>'hidden','name'=>'id', 'id'=>'id_producto1', 'value'=>$prod->codigo));
                                        echo input(array('type'=>'hidden','name'=>'segment','id'=>'segmento1', 'value'=>$this->uri->segment(4)));
                                        echo input(array('type'=>'hidden','name'=>'select_precio',  'id'=>'precio1',  'value'=>number_decimal($prod->precioOferta_sin_iva)));
                                        if($prod->stock_bodega > 0){
                                            echo tagcontent('a','Agregar'.  tagcontent('i','',array('class'=>'fa fa-shopping-cart')),array('class'=>'add-to-cart-link','id'=>'ajaxformbtn','data-target'=>'res_info','onclick'=>'agregarProducto();'));
                                        }else{
                                            echo tagcontent('a','Agregar'.  tagcontent('i','',array('class'=>'fa fa-shopping-cart')),array('class'=>'add-to-cart-link','id'=>'ajaxformbtn','data-target'=>'res_info'));
                                        }
                                   // echo Close('form');
                                        
                                 echo Close('div');
                             echo Close('div');
                              echo " <h2>".get_short_string($prod->nombreUnico , 50 , false)."</h2>";
                              echo Open('div',array('class'=>'product-carousel-price'));
                              if(number_decimal($prod->precioNormal_con_iva) > 300){
                                        $span_cuotas = '<br>Ó 18 Coutas de: $'.  number_decimal($prod->cuota);
                                    }else{
                                        $span_cuotas = '';
                                    }
                                    echo tagcontent('ins','P. HOY: $'. number_decimal($prod->total_desc),array('class'=>'text-success'));
...
function agregarProducto(){
	...
	var segmento=$('#segmento1').val();
	var precio1=$('#precio1').val();
    		//alert(precio1);
        $.post(main_path+'shopping_cart/cart/add_product',
	     {'id': id_producto,
	      'segmento':segmento,
	      'select_precio':precio1
	 		}, 
	 	function(data){ 
                    //alert(precio1);
	 		});
